...
exports.saveEquipo = (req, res) => {
  ...
  exports.updateEquipo = (req, res) => {
    ...
    const nombreEquipo = req.body.nombreEquipo;
      ...
      conexion.query("UPDATE equipos SET ? WHERE id = ?", [{nombreDT: nombreDT, nombreEquipo: nombreEquipo}, id], (error, results) => {
        if (error) {
          console.log(error);
        res.status(400).json({"msg": "error"});;
        } else {
...
exports.saveCarrera = (req, res) => {
  ...
  conexion.query('SELECT nombreCarrera FROM carreras WHERE nombreCarrera=?',[nombreCarrera] , (error, carrera) => {
    if (carrera.length == 1) {
      res.status(400).json({"msg": "error"});
    } else {
